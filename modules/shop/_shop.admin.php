...
EOF;
		...
		if($edit && !$this->ci->fb->submit){
			...
			$this->ci->fb->change("sizes[]",array("value"=>$this->d['item_res']->sizes));
...
EOF;
		...
		$this->ci->fb->add("input:text",array(
			"label"=>"Фамилия получателя посылки",
			"name"=>"delivery_name",
			"parent"=>"greed",
...
EOF;
		...
		foreach($this->d['order_res']->basket->products AS $product_id=>$r)
		{
			...
			$order_info.=<<<EOF
<tr>
	<td><a target="_blank" href="{$this->admin_url}?m=shop&a=edit_product&id={$r->id}">{$product->title}</a>{$suppliers}</td>
...
EOF;
		...
		if($this->d['order_res']->delivery_method!=0 && $this->d['order_res']->delivery_method!=2){
			$order_info.=<<<EOF
<br /><br />
<strong>Доставка:</strong> {$this->d['order_res']->discount->delivery_hmn}
<br /><br />
<strong>Цена с доставкой:</strong> {$this->d['order_res']->discount->price_total_hmn}
EOF;
		}
...
EOF;
	...
	public function export()
	{
		...
		$date=date("d")." ".$months[intval(date("m",$this->d['order_res']->date_update))-1]." ".date("Y",$this->d['order_res']->date_update)." р.";
...
<tr>
	...
	<td valign="top">
		<p>Империя Посуды</p>
		<p>тел. (044)599-2-555</p>
		<p>Адреса м.Киев, вул.Ромена Роллана 7 оф.232</p>
	</td>
</tr>
<tr>
...
<tr>
	...
	<th bgcolor="#CDCDCD">Од.</th>
	<th bgcolor="#CDCDCD">Кількість</th>
	<th bgcolor="#CDCDCD">Ціна без ПДВ</th>
	<th bgcolor="#CDCDCD">Сума без ПДВ</th>
</tr>
EOF;
...
EOF;
		...
		$num2str=num2str($this->d['order_res']->basket->total_amount);
$html.=<<<EOF
<tr>
	<td colspan="4" style="border:none; border-right:1px solid #000;"></td>
	<td align="right"><strong>Разом без ПДВ:</strong></td>
	<td align="right">{$this->d['order_res']->basket->total_amount_hmn}</td>
</tr>
<tr>
	<td colspan="4" style="border:none; border-right:1px solid #000;"></td>
	<td align="right"><strong>Доставка{$delivery_method_hmn}:</strong></td>
	<td align="right">{$this->d['order_res']->discount->delivery_hmn}</td>
</tr>
<tr>
	<td colspan="4" style="border:none; border-right:1px solid #000;"></td>
	<td align="right"><strong>ПДВ:</strong></td>
	<td align="right"></td>
</tr>
<tr>
	<td colspan="4" style="border:none; border-right:1px solid #000;"></td>
	<td align="right"><strong>Всього з ПДВ та доставкою:</strong></td>
	<td align="right">{$this->d['order_res']->discount->price_total_hmn}</td>
</tr>
</table>
...
<br />
<center>Товар можливо повернути протягом 14 днів за  умови збереження товарного вигляду товару та упаковки, при наявності даного документу.</center>
EOF;
