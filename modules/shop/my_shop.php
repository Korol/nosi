...
// 		die("\n\n<hr />OK!");
    ...
    public function category2()
    {
        error_reporting(1);
        // ID категории
        $category_id = $this->url_structure_res->extra_id;
        // массив ID категорий для запроса товаров
        $category_in = array((int)$category_id);
        // инфа о категории
        $category_info = $this->db
            ->select('categoryes.id, url_structure.url, url_structure.title')
            ->join('url_structure', 'url_structure.extra_id = categoryes.id')
            ->where(array(
                'categoryes.id' => $category_id,
                'categoryes.show' => 1,
        ...
        $category_childs = $this->fetch_recursive($all_categories, $category_id, true);
        // массив ID дочерних категорий
        $subcat_ids = array();
        if(!empty($category_childs)){
            $this->load->helper('functions');
            // реструктурируем массив, в ключи загоняем ID подкатегорий
            $category_childs = toolIndexArrayBy($category_childs, 'id');
            // получаем массив ID подкатегорий для запросов товаров
            $subcat_ids = array_keys($category_childs);
        }
        // сливаем оба массива с ID категорий в один, для WHERE IN ()
        $category_in = array_merge($category_in, $subcat_ids);


//        var_dump($category_in);
    }

    /**
     * @param array $src_arr - массив категорий
     * @param int $currentid - ID, для которого нужно найти потомков
     * @param bool $parentfound - флаг, false - в результат включена инфа и об $currentid, true - не включена
     * @param array $cats - результирующий массив
     * @return array
     */
...
//        var_dump($where, $d);
        ...
        if ($this->url_structure_res->options->category_id > 0) {
            ...
            /**
             * Новая логика – выборка товаров не только по текущей категории – но и по подкатегориям каталога
             */
            $category_id = $this->url_structure_res->options->category_id;
            $category_in = array((int)$category_id);
            // получаем все категории
            $all_categories = $this->db
                ->select('categoryes.id, categoryes.parent_id, categoryes.title, url_structure.url')
...
//                "shop_products_categories_link", "shop_products_categories_link.product_id = shop_products.id && shop_products_categories_link.category_id IN(" . $this->url_structure_res->options->category_id . ")",
                "shop_products_categories_link", "shop_products_categories_link.product_id = shop_products.id && shop_products_categories_link.category_id IN(" . implode(", ", $category_in) . ")",
            );
        }
        ...
        // забиваем отмеченные фильтры в массив для использования на странице
        ...
        if ($this->category_res->options->disable_filter != 1) {
            // получаем данные по фильтрам - нахрена-то выбираем прям все over 500 товаров из БД ?!?!?!?
            // не 30 на страницу – а тупо ВСЕ товары из текущей категории, Карл!!!! ппц...
            ...
            // крутим все эти over 500 товаров, чтоб получить 4-5 типов товара – вместо простого SQL-запроса(((
            foreach ($this->d['products_filter_res'] AS $r) {
                // собираем тип товаров – непонятно зачем?
                if (!in_array($r->type_id, $exists_filters)) {
                    $exists_filters[] = $r->type_id;
                } else {
                    continue;
                }
                // получаем фильтры и параметры для данного типа товаров
                $r->fields = $this->db
                ...
                // проходим по полученным фильтрам – чтоб по ним составить условие для товаров
                // если какие-то из этих фильтров отмечены пользователем
                foreach ($r->fields AS $field) {
                    // декодим параметры фильтров из JSON
                    if (is_string($field->params)) {
                        $field->params = json_decode($field->params);
                    }
                    // если этот фильтр отмечен пользователем
                    if (isset($this->d['filter_selected'][$field->id])) {
                        // проверяем зачем-то его тип – хотя в 100% случаев это select
                        switch ($field->field_type) {
                            case'select':
                                $ids = array(); // сюда собираем ID тех параметров, которые выбрал пользователь в фильтре
                                ...
                                // если такие ID есть – строим по ним условие для запроса и подсчета количества товаров
                                if (sizeof($ids) > 0) {
            ...
            if ($this->category_res->options->disable_filter != 1) {
                ...
                // для фильтров на странице нужны переменные:
                // $products_filter_res - фильтры
                // $filter_selected - отмеченные фильтры

                // получаем типы товаров в категории
                $product_types = $this->db->distinct()->select('shop_products.type_id')
                    ->join('shop_products_categories_link', 'shop_products_categories_link.product_id = shop_products.id')
                    ->join("shop_product_types", "shop_product_types.id = shop_products.type_id && shop_product_types.show = 1")
                    ->where('shop_products_categories_link.category_id', $this->d['category_res']->id)
                ...
                if (!empty($product_types)) {
                    ...
                    $filters = $this->db->where_in('type_id', $type_ids)->where('filter', 1)->get('shop_product_type_fields')->result();
                }

                // сверяем полученные фильтры с отмеченными пользователем на странице
                // если есть совпадения - то строим соответствующие условия
                if (!empty($filters)) {
//                    $this->d['products_filter_res'] = array();
                    $temp_array = array();
                    foreach ($filters as $filter) {
                        // декодим параметры фильтров из JSON
                        if (is_string($filter->params)) {
                            $filter->params = json_decode($filter->params);
                        }

                        // формируем данные для показа фильтров на странице
                        $temp_array[$filter->type_id]['fields'][] = $filter;

                        // если этот фильтр отмечен пользователем
                        if (isset($this->d['filter_selected'][$filter->id])) {
                            // проверяем зачем-то его тип – хотя в 100% случаев это select
                            switch ($filter->field_type) {
                                case'select':
                                    $ids = array(); // сюда собираем ID тех параметров, которые выбрал пользователь в фильтре
                                    foreach ($filter->params->options AS $value => $option) {
                                        if (!in_array($value, $this->d['filter_selected'][$filter->id]))
                                            continue;
                                    ...
                                    // если такие ID есть – строим по ним условие для запроса и подсчета количества товаров
                                    if (sizeof($ids) > 0) {
                                        $products_query->where("`f_" . $filter->id . "` IN(" . implode(",", $ids) . ")");
                                        $products_query_num->where("`f_" . $filter->id . "` IN(" . implode(",", $ids) . ")");
                                    }
                                    break;
                            }
                        }
                    ...
                    if(!empty($temp_array)){
                        foreach($temp_array as $k => $v){
                            $this->d['products_filter_res'][] = (object)$v; // формируем структуру для показа фильтров
                        }
                    }
        ...
        $this->d['products_num'] = $products_query_num
                ->where($this->d['where'])
                ->get()->row()->numrows; // added
//                ->count_all_results(); // commented
        ...
        // общее количество страниц с товарами, с учетом фильтров
...
//        }
        ...
        $this->d['products_res'] = $products_query
                ...
                ->result();
// 9
        // делаем ещё 100500 запросов, чтоб дополнить товары всякими параметрами - REFACTOR THIS!!!!
...
// 100
        ...
        if($version == 'old'){ // version+
            ...
            // БЛЯ!!!! ОПЯТЬ ПОЛУЧАЕМ ВСЕ 100500 ПРОДУКТОВ В КАТЕГОРИИ!!!
            // ЧТОБ ПРОСТО ТУПО БЛЯ ПОЛУЧИТЬ ID БРЕНДОВ ПРОДУКТОВ В КАТЕГОРИИ
            // опять таки – вместо того, чтоб сделать ОДИН простенький SQL-запрос
            ...
            if (sizeof($exists_products_manufacturers) > 0) {
                // А ТЕПЕРЬ ВСЕ ЭТИ 100500 ПОВТОРЯЮЩИХСЯ brand_id ЗАПИХНЁМ В WHERE IN (...) - ЭТО ПИЗДЕЦ, ГОСПОДА!!!!!!!))))
                // чтоб просто получить о них информацию
            ...
            // получаем ID брендов товаров в категории
            $this->d['manufacturers_res'] = $this->db->distinct()->select('shop_products.brand_id, categoryes.id, categoryes.title')
                ->join('shop_products_categories_link', 'shop_products_categories_link.product_id = shop_products.id')
                ->join('categoryes', 'categoryes.id = shop_products.brand_id')
                ->where(array(
                    'categoryes.type' => 'shop-manufacturer',
                    'categoryes.show' => 1,
                    'shop_products_categories_link.category_id' => $this->d['category_res']->id
                ))
        ...
        // как результат – 680(!!!!11111) запросов для того, чтоб вывести на страницу 30 товаров и десяток фильтров
        // адский ад, товарищи!!!
        // OLD:
        // MEMORY USAGE:  27,547,496 bytes
        // DATABASE QUERIES: 685
        // NEW:
        // на этом этапе идет около 100 запросов к БД
        // самая жопа начинается в views (structure – там в Меню верхнем капец что творится, до 400 запросов + product_list тоже до 100 запросов)
        // короче ппц полный!
