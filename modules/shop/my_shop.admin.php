...
//
		...
		if($this->ci->fb->submit){
			...
			if(sizeof($this->d['global_errors'])==0){
				$category_ids=array(); // Категории из POST
				$category_all_ids=array(); // ID всех Категорий (из POST + их иерархия)
				$category_paths=array(); // Категории из POST с их иерархией для JSON
...
EOF;
	...
	{
        // удаляем из таблицы связей товар-категория (shop_products_categories_link)
        // связи с категориями, которые уже не существуют в Каталоге
		$this->repair_categories();

        // получаем ID всех товаров в Каталоге
		$products_res=$this->db
		->select("id")
		->get("shop_products")
		->result();
		...
		foreach($products_res AS $r)
		{
            ...
            // согласно текущей структуре каталога
