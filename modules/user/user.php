...
class userModule extends Cms_modules {
		...
		if($this->input->post("register_sm")!==false){
			...
			if(sizeof($errors)==0){
				...
				$additional_data=array(
					...
					"accept_email_notifications"=>$this->input->post("accept_email_notifications")==1?1:0,
					"active"=>1
				);
				...
				if($user_id===false){
					print implode("\n",$this->ci->ion_auth->errors_array());
					exit;
				}else{
					// автовход сразу после регистрации
					$this->ci->ion_auth->login($this->input->post("email"),$this->input->post("password"),true);
				}
			}
	...
	{
		if(!$this->ci->ion_auth->forgotten_password($this->input->post("email"))){
			print implode("\n",$this->ci->ion_auth->errors_array());
			exit;
		}
