...
<?php
$ym_user_ip = $this->input->ip_address();
if($ym_user_ip !== '127.0.0.1'):
?>
...
<script type="text/javascript">
    (function (d, w, c) {
        (w[c] = w[c] || []).push(function() {
            try {
                w.yaCounter35652455 = new Ya.Metrika({
                    id:35652455,
                    clickmap:true,
                    trackLinks:true,
...
<noscript><div><img src="https://mc.yandex.ru/watch/35652455" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
<!-- /Yandex.Metrika counter -->
<?php
endif;
?>
	...
	<div class="popup modal hide fade" id="popupRemind">
		...
		<div class="modal-body">
			<div class="form">
					...
					<div class="formRow">
						<label>Email: <span class="necessary">*</span></label><input type="text" name="email" value="" />
					</div>
			...
			<div id="head">
				...
				<div class="mainmenu">
					<ul>
						<?php
                        // q:171
                        $items=$this->menu->get_menu_items(24);// 24 // q:290
                        foreach($items AS $r)
						{
							...
							?><li  class="item<?php print $r->id; ?>"><a<?php print $a; ?> href="<?php print $r->link; ?>"><span><?php print $r->title; ?></span></a><?php
							    $child=$this->menu->get_menu_items($r->id, false); // q:599
								if($child){
									...
									foreach($child as $c){
										...
										?>
                                        <li class="item<?php print $c->id; ?>">
                                            <a<?php print $a; ?> href="<?php print $c->link; ?>"><span><?php print $c->title; ?></span></a>
                                            <?php
                                            ////////
                                            $sub_child = $this->menu->get_menu_items($c->id);
                                            if(!empty($sub_child)){
                                                echo '<ul>';
                                                foreach($sub_child as $sch){
                                                    $a=in_array($sch->id,$active_menu_items)?' class="active"':'';
                                                    $sch->title=str_replace(" ","&nbsp;",$sch->title);
                                                    echo '<li><a' . $a . ' href="' . $sch->link . '"><span>' . $sch->title . '</span></a></li>';
                                                }
                                                echo '</ul>';
                                            }
                                            ////////
				...
				<div class="search">
					<form method="get" action="/search.html">
						...
						<input type="text" name="keywords" class="keyword" />
					</form>
				</div>
                                <!--div class="ny"></div-->
				<div class="clear"><!-- --></div>
			...
			<div class="footMenu">
				...
				<ul>
					...
					foreach($items AS $r)
					{
						$a=in_array($r->id,$active_menu_items)?' class="active"':'';
						$r->title=str_replace(" ","&nbsp;",$r->title);
						?><li><a<?php print $a; ?> href="<?php print base_url($r->link); ?>"><span><?php print $r->title; ?></span></a></li><?php
			...
			<div class="footMenu">
				...
				<ul>
					...
					foreach($items AS $r)
					{
						$a=in_array($r->id,$active_menu_items)?' class="active"':'';
						$r->title=str_replace(" ","&nbsp;",$r->title);
                                                ?><li><a<?php print $a; ?> href="<?php print $r->link; ?>" target="_blank"><span><?php print $r->title; ?></span></a></li><?php
