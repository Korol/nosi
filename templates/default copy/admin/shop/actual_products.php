...
function calcProductsHash()
{
	...
	$("input[name^='product_original_price']").each(function(){
		...
		$(this).parents("td:eq(0)").next().text(original_sum_hmn+" грн.");

		$(this).parents("td:eq(0)").nextAll("td:eq(4)").text(profit+" грн.");
	});
	...
	$("#table-default tr:last td:eq(5)").text(price_format(total_product_original_price)+" грн.");
	$("#table-default tr:last td:eq(6)").text(price_format(total_product_original_sum)+" грн.");
	$("#table-default tr:last td:eq(8)").text(price_format(total_price)+" грн.");
	$("#table-default tr:last td:eq(9)").text(price_format(total_sum)+" грн.");
	$("#table-default tr:last td:eq(10)").text(price_format(total_profit)+" грн.");
